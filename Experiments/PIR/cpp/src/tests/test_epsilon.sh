...
access_specific_block() {
    echo -e "\nTest A2: Client is accessing one specific block:\n"
...
access_all_blocks() {
    ...
    for (( i=1; i<=sqrt_N; i++ )); do
        #Wait a little more than the server_delta
        sleep 5
...
LOG_FILE="${log_dir}/test_epsilon_${TIMESTAMP}.txt"

EXECUTABLE="${bin_dir}/server_epsilon"
