...
access_specific_block() {
    local idx=$1
    echo -e "\nTest A2: Accessing block: $idx from the remote database:\n"
}

access_all_blocks() {
    echo -e "\nTest A3: Accessing all the blocks from the remote database:\n"
...
LOG_FILE="${log_dir}/test_client_${TIMESTAMP}.txt"

EXECUTABLE="${bin_dir}/pir_client"
