...
static int OneTimeInit_alpha() {
    ...
    export_to_file_from_mpz_class(ONE_TIME_MATERIALS_LOCATION_ALPHA + "pk_E_dashed.bin", pk_E_dashed);
...
static int PerEpochOperations_alpha(){
    ...
    pk_E_dashed = import_from_file_to_mpz_class(ONE_TIME_MATERIALS_LOCATION_ALPHA + "pk_E_dashed.bin");
...
start:
    ...
    h_alpha3 = ElGamal_randomGroupElement();

    /* 2.1 Compute E(T_*.a_dashed) */
    T_star_a_dashed = ((T_star * a_dashed) % p);
    E_T_star_a_dashed = ElGamal_encrypt(T_star_a_dashed, pk_E);

    /* 2.2.1 Send the first componet of E(T_*.a_dashed) to Server Gamma */
...
static int ProcessClientRequest_alpha(){
    ...
    if (FHEcryptoContext->DeserializeEvalMultKey(emkeys, SerType::BINARY) == false) {
        PrintLog(LOG_LEVEL_ERROR, __FILE__, __LINE__, "Server Alpha: Cannot dserialized multikey file");
        return -1;
    }
